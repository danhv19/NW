import matplotlib
matplotlib.use('Agg') # Evita problemas de GUI en el servidor
...
def plot_historical_results(df, target_col, features, plots_folder):
    ...
    try:
        ...
        # Seleccionar solo columnas numéricas de 'features' y el 'target'
        numeric_cols_for_corr = df[features].select_dtypes(include=np.number).columns.tolist()
        if target_col not in numeric_cols_for_corr:
             numeric_cols_for_corr.append(target_col)
             
        corr_matrix = df[numeric_cols_for_corr].corr()
        # Tomar solo la correlación con la columna objetivo
        corr_with_target = corr_matrix[[target_col]].sort_values(by=target_col, ascending=False)
        
        sns.heatmap(corr_with_target, annot=True, cmap="coolwarm", vmin=-1, vmax=1, fmt='.2f')
    ...
    if 'Carrera' in df.columns:
        try:
            ...
            career_success = df.groupby('Carrera')[target_col].mean().sort_values(ascending=False).head(15) # Top 15
            sns.barplot(x=career_success.values, y=career_success.index, palette="viridis")
            plt.title('Tasa de Aprobación por Carrera (Top 15)')
    ...
    if 'EDAD' in df.columns:
        try:
            plt.figure(figsize=(10, 6))
            sns.kdeplot(df[df[target_col] == 1]['EDAD'], label='Aprobado', fill=True, clip=(0, 80))
            sns.kdeplot(df[df[target_col] == 0]['EDAD'], label='Desaprobado', fill=True, clip=(0, 80))
